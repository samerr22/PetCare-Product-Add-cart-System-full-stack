...
export default function CreatePost() {
  ...
  const [imageUploadProgress, setImageUploadProgress] = useState([null, null, null]); // State for three upload progress
  const [imageUploadErrors, setImageUploadErrors] = useState([null, null, null]); // State for three upload errors
        ...
        {[0, 1, 2].map((index) => (
          <div key={index} className="flex gap-4 items-center justify-between border-4 border-teal-500 border-dotted p-3">
            ...
            {imageUploadErrors[index] && (
              <p className="mt-5 text-red-600 bg-red-300 w-300 h-7 rounded-lg text-center">{imageUploadErrors[index]}</p>
            )}
            {formData[`image${index + 1}`] && (
              <img src={formData[`image${index + 1}`]} alt={`upload-${index + 1}`} className="w-full h-72 object-cover" />
            )}
          </div>
        ))}

        {publishError && (
          <p className="mt-5 text-red-600 bg-red-300 w-300 h-7 rounded-lg text-center">{publishError}</p>
        )}
